...
async function loadAllIssue() {
  ...
  data.data.forEach((issue) => {
    ...
    div.className =
      'p-5 border-t-5  rounded-2xl bg-slate-500 max-w-100 space-y-3 transform transition duration-300 hover:scale-103 hover:cursor-pointer hover:bg-slate-600 animated';
  ...
  async function openIssue() {
    ...
    for (const open of issues) {
      if (open.status === 'open') {
        ...
        const div = document.createElement('div');
        div.className =
          'p-5 border-t-5 border-green-600 rounded-2xl bg-slate-500 max-w-100 space-y-3 card transform transition duration-300 hover:scale-103 hover:cursor-pointer hover:bg-slate-600 animated';
        div.innerHTML = `
  ...
  async function closedIssue() {
    ...
    for (const closed of issues) {
      if (closed.status === 'closed') {
        ...
        const div = document.createElement('div');
        div.className =
          'p-5 border-t-5 border-[#A855F7] rounded-2xl bg-slate-500 max-w-100 space-y-3 card transform transition duration-300 hover:scale-103 hover:cursor-pointer hover:bg-slate-600 animated';
        div.innerHTML = `
